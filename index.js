...
passport.use(new twitchStrategy({clientID:TClientid, clientSecret: TClientsecret, callbackURL: 'https://tcpd.noeul.codes/auth/twitch', scope: 'user_read channel:read:redemptions'}, async (accessToken, refreshToken, profile, done) => {
  let user
...
app.post('/upload/img', auth, upload.single('image'), async (req, res) => {
...
app.get('/dashboard', auth, acctoken, async (req, res) => {
  fetch('https://api.twitch.tv/helix/channel_points/custom_rewards?broadcaster_id=' + req.user.id, {method:'GET', headers: { 'Client-Id': TClientid, 'authorization': 'Bearer ' + req.oauth }}).then(res => res.json()).then( async json => {
